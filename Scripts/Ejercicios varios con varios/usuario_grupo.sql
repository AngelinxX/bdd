...
insert into usuario_grupo values (1,1,8),
								 (2,2,3),
								 (3,3,8),
								 (4,4,9),
								 (5,5,1),
...
select us.nombre
from usuarios us
where us.id in( select ug.us_id
from usuario_grupo ug where ug.gr_id = 1
)
...
from usuarios us 
where us.id in (select ug.us_id
				from usuario_grupo ug where ug.gr_id = 2)


select gr.nombre, max(ug.us_id)
from grupo gr, usuario_grupo ug
where gr.id = ug.gr_id
group by gr.nombre

select gr.nombre, min(ug.us_id)
from grupo gr, usuario_grupo ug
where gr.id = ug.gr_id
group by gr.nombre
...
from usuarios us 
where us.id in (select ug.us_id
				from usuario_grupo ug where ug.gr_id = 3)
